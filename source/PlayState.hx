...
class PlayState extends MusicBeatState
{
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					else
					{
						...
						{
							var video:MP4Handler = new MP4Handler();
							video.playMP4(Paths.video('post-evacuate'), new StoryMenuState(), false, false, false); 
						}
						else if (PlayState.storyDifficulty == 3)
						{
							var video:MP4Handler = new MP4Handler();
							video.playMP4(Paths.video('troll'), new Killer(), false, false, false);
						} 
					}
				...
				else
				{
					...
					switch (SONG.song.toLowerCase())
					{
						...
						{	
							...
							paused = true;
							var video:MP4Handler = new MP4Handler();
							video.playMP4(Paths.video('prepare'), new PlayState(), false, false, false); 
						}
						...
						case 'prepare':
						{
							...
							paused = true;
							var video:MP4Handler = new MP4Handler();
							video.playMP4(Paths.video('evacuate'), new PlayState(), false, false, false); 
						}
					}
	...
	private function keyShit():Void // I've invested in emma stocks
	{
		...
		if (holdArray.contains(true) && /*!boyfriend.stunned && */ generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				if (daNote.isSustainNote && daNote.canBeHit && daNote.mustPress && holdArray[daNote.noteData] && daNote.sustainActive  && daNote.noteStyle != 'warning')
				{
		...
		if ((KeyBinds.gamepad && !FlxG.keys.justPressed.ANY))
		{
			...
			if (pressArray.contains(true) && generatedMusic)
			{
				...
				notes.forEachAlive(function(daNote:Note)
				{
					if (daNote.noteStyle != 'warning' && daNote.canBeHit && daNote.mustPress && !daNote.tooLate && !daNote.wasGoodHit && !directionsAccounted[daNote.noteData])
					{
		...
		notes.forEachAlive(function(daNote:Note)
		{
			if (PlayStateChangeables.useDownscroll && daNote.y > strumLine.y || !PlayStateChangeables.useDownscroll && daNote.y < strumLine.y)
			{
				...
				if (daNote.noteStyle != 'warning' && PlayStateChangeables.botPlay && daNote.canBeHit && daNote.mustPress || daNote.noteStyle != 'warning' && PlayStateChangeables.botPlay && daNote.tooLate && daNote.mustPress)
				{
					if (loadRep)
					{
